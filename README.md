...
```
GET /health/live    — процесс жив, без обращения к зависимостям (OBS-003)
GET /health/ready   — готовность; Degraded отдаёт 200 и остаётся в ротации (OBS-005)
GET /metrics        — Prometheus; без аутентификации, граница — сеть (OBS-007)
```

Правила алертов разделов 30.3 и OBS-012 лежат в [`deploy/prometheus/alerts.yml`](./deploy/prometheus/alerts.yml).
Высококардинальные labels запрещены (`OBS-010`) и проверяются тестом `TEST-SEC-023` по реальному
скрейпу `/metrics`.

### Резервное копирование

```
scripts/backup/backup.sh          # ежедневный дамп + зеркало бакета (REL-001, REL-002)
scripts/backup/restore-drill.sh   # ежемесячная проверка восстановления (REL-005)
scripts/db/02-tenant-integrity.sql # проверки арендной целостности после восстановления (TEN-095)
```

Невыполненная проверка восстановления считается инцидентом эксплуатации, а не задолженностью
(`REL-005`).
