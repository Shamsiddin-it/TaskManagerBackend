...
| 12 | `SCH-008` объявляет `AutoGenerationKey` как `varchar(120)` | **Колонка расширена до 160** | **Противоречие внутри ТЗ.** Шаблон ключа из `SCH-009` — четыре GUID в формате `N`, дата и источник — даёт ровно 147 символов и в 120 не помещается: выполнить оба требования буквально невозможно. Приоритет отдан шаблону, потому что `SCH-023` требует его явно и с обоснованием (ключ читается человеком при разборе инцидента без обращения к БД), тогда как за длиной колонки никакого требования не стоит. Длина закреплена модульным тестом |

| 13 | Состав полей `AiSummary` (раздел 10.16) | **Добавлена колонка `last_forced_at`** (`timestamptz`, nullable) и индекс `ix_ai_summaries_forced` | **Пробел в ТЗ, обнаружен при реализации `AI-011`.** Лимит принудительной регенерации задан «не чаще раза в сутки **на субъекта отчёта**», а субъект живёт в нескольких строках: у каждого периода свой `CacheKey`, значит своя строка. Ни одна документированная колонка не выражает «этот субъект уже регенерировали сегодня», поэтому лимит можно было бы применять только к строке — а это другой лимит: он снимается сдвигом периода на один день. Колонка минимальна и не влияет ни на `CacheKey`, ни на форму scope |
| 14 | `TEN-048` перечисляет действия, у которых `branch_id` может быть NULL | **В список добавлено `ai.summary_generate`** (аналогично `audit.read`, отклонение из фазы 7) | **Противоречие внутри ТЗ.** `TEN-078` определяет сводное резюме организации явно — `Scope='Organization'`, и `ck_ai_summaries_scope_shape` требует для него `branch_id IS NULL`; `AI-021` требует записывать в AuditLog **каждый** вызов провайдера. С `ai.summary_generate` вне списка `TEN-048` эти два требования исключают друг друга: запись такого вызова падает на `ck_audit_logs_branch_scope`. Как и `audit.read`, действие **не** принудительно обнуляется: резюме филиала несёт свой филиал, без филиала остаётся только сводное |
| 15 | Feature flag `Features:AiSummary` (раздел 4.1) | **Флаг называется `Ai:Enabled`** | То же решение и та же причина, что для Telegram (отклонение фазы 13): остальные настройки интеграции лежат в секции `Ai`, и держать переключатель отдельно от того, чем он управляет, — приглашение включить одно без другого |
...
Замечания по реализации:
- **Клиент MinIO, а не AWS SDK.** Изначально была взята AWS SDK — ТЗ говорит «S3-совместимое хранилище (MinIO)», и `ResponseHeaderOverrides` в presigned URL закрывали `SEC-017`. Позже клиент заменён на вендорский: хранилище в ТЗ названо поимённо, а `response-*` параметры MinIO подписывает тем же способом, поэтому ничего не потеряно.
- **Обходной путь со схемой presigned URL удалён вместе с AWS SDK.** Резолвер эндпоинтов SDK v4 строил presigned URL по `https` независимо от схемы `ServiceURL`, и против MinIO по HTTP клиент падал с ошибкой TLS-фрейминга; URL приходилось переписывать после подписи. Клиент MinIO берёт схему из собственной конфигурации, и переписывать нечего.
...
**Выполнено.** 737 тестов зелёные (287 unit / 64 arch / 386 integration), из них 24 интеграционных и 5 модульных — по этой фазе.

Замечания по реализации:
- **Найден дефект в BDA: `ORDER BY` по возрастанию в PostgreSQL ставит NULL последними.** `SCH-013` требует обратного — «null считается самым ранним, новичок получает задачу первым». Сортировка теперь идёт сначала по `HasValue` (false раньше true), что даёт нужный порядок переносимо, без сырого `NULLS FIRST`. Поймано тестом «ментор без назначений идёт первым».
- **`AutoGenerationKey` расширен до `varchar(160)`** — см. отклонение 12 в разделе 0.
- **Outbox остался `BackgroundService`, а не задачей Hangfire.** Отклонение фазы 12 сохраняется: это непрерывно вычерпываемая очередь, которая сама себя тактирует, а `SKIP LOCKED` уже даёт то, ради чего 20.1 требует `DisableConcurrentExecution`. Остальные пять задач таблицы 20.1 — recurring job'ы Hangfire.
- **Джоб просрочки не трогает задачи закрытого филиала** (`BRN-033`): там некому реагировать, а перевод в `Overdue` создал бы метрику о филиале, который не работает.
- **`TaskEvent.RecordSystem` добавлен для условного UPDATE.** Джоб просрочки двигает строки сырым SQL ради идемпотентности (`SCH-007`), поэтому отслеживаемого `Assignment` для `NextEventSequence()` нет — номер приходит из того же оператора, который его и выделил.
- **Регистратор снимает job'ы для зон, которых больше нет ни у одной категории.** Иначе смена часового пояса категории оставила бы старую задачу генерировать по прежнему локальному часу навсегда.
- **Hangfire Dashboard не монтируется нигде** (`TEN-059`), схема — отдельная `hangfire`, чтобы EF-миграции её не диффали (ADR-002).
- **Попутно поднят MailKit до 4.17.0**: 4.14.0, добавленный в фазе 12, нёс известную уязвимость средней важности.
...
**Выполнено.** 758 тестов зелёные (287 unit / 64 arch / 407 integration), из них 21 интеграционный — по этой фазе.

Замечания по реализации:
- **Метрики считаются одним SQL-запросом, а не LINQ.** `ANA-008` требует медиану, а `percentile_cont(0.5) WITHIN GROUP` в LINQ не транслируется; раз медианы всё равно в SQL, остальные девять метрик считаются тем же проходом, а не десятью.
- **Разные метрики относят задачу к периоду по разным датам** (`ANA-003`), поэтому счётчики построены на `FILTER (WHERE ...)`, а не на одном общем `WHERE`.
- **Найден дефект: запрос уничтожал соединение DbContext.** `await using` на `GetDbConnection()` закрывал чужой ресурс, и любой следующий запрос в том же запросе падал с `ObjectDisposedException`. Соединение теперь открывается и закрывается через EF и не диспозится.
- **Группировка задаётся закрытым перечислением, а не строкой.** Колонки группировки интерполируются в SQL, поэтому всё, на что мог бы повлиять вызывающий, туда не попадает вовсе; имени категории среди вариантов нет по построению (`TEN-071`).
- **Отчёт без разбивки не отдаёт строку-дубль.** Запрос с группировкой `None` возвращает одну строку со всеми ключами `NULL`; отдавать её как строку разбивки значило бы повторить итог под строкой, которую нечем идентифицировать — а для обезличенного отчёта ментора именно такой строки быть не должно.
- **Порог приватности проверяется до вычисления любых цифр** (`ANA-012`): частичный ответ с последующим отказом — это и есть разностная атака, которую правило закрывает.
- **Отрицательные длительности исключаются из агрегатов и считаются счётчиком** (`ANA-007`): отчёт, молча выбросивший строки, и отчёт, где таких строк нет, выглядят одинаково, а означают разное.
...
**Выполнено.** 800 тестов зелёные (310 unit / 64 arch / 426 integration), из них 19 интеграционных и 22 модульных — по этой фазе.

Замечания по реализации:
- **Метрики берутся у `IAnalyticsService`, а не считаются заново.** Резюме обязано говорить о тех же числах, что показывает страница аналитики, а правила scope — прежде всего порог пяти менторов `ANA-012` — применяются там. Второй путь запроса означал бы второй набор правил, который рано или поздно разойдётся, и первое же расхождение — резюме со ссылкой на цифры, которых читатель не находит.
- **Field allowlist выражен типом `AiPromptInput`, а не проверкой.** Доменную сущность нельзя передать провайдеру, потому что провайдер принимает этот тип; чтобы отправить запрещённое поле, нужно завести свойство в файле, единственное назначение которого — перечислить разрешённое (`AI-006`). Интеграционный тест проверяет отсутствие UUID, ФИО, `Slug` и `Code` в том, что реально ушло бы наружу, а не в намерениях кода (`TEN-079`).
- **Ключ кэша собирается через разделитель, а не конкатенацией, как записано в формуле.** Формула смешивает GUID фиксированной длины с литералами переменной (`NONE`, `ALL_BRANCHES`) и с датами; конкатенация частей переменной длины — ровно тот способ, которым два разных входа получают один ключ, то есть та самая коллизия, против которой ключ и введён.
- **Порядок проверок важен не меньше самих проверок.** `TEN-077` требует отказать Branch Admin'у до обращения к кэшу и до провайдера: 404, выданный после чтения кэша, всё равно раскрывает по времени ответа, что отчёт существует.
- **Branch id от кого угодно, кроме Organization Admin, отклоняется, а не игнорируется.** Эндпоинты метрик его игнорируют — и могут: они отвечают цифрами, про которые видно, что они свои. Резюме — это текст, и текст про главный офис под заголовком «Худжанд» хуже, чем отсутствие ответа.
- **Scope персонального отчёта берётся у субъекта, а не из запроса.** Персональный отчёт — про работу одного человека, а работа происходила в его филиале и его категории; позволить вызывающему назвать другую пару значило бы завести строку под scope, которому цифры не принадлежат, причём `ck_ai_summaries_scope_shape` был бы доволен.
- **Retry принадлежит провайдеру, а не SDK.** SDK по умолчанию повторяет дважды со своим backoff; оставленный включённым, он молча умножил бы число попыток и вышел бы за 90-секундный бюджет `AI-003`, продолжая выглядеть послушным. Клиент создаётся с `MaxRetries = 0`.
- **Thinking выключен явно.** `AI-002` ограничивает ответ 1500 токенами, а на модели, где thinking включён по умолчанию, этот бюджет общий: резюме обрывалось бы на середине фразы рассуждением, которого читатель не видит. Многошаговое рассуждение здесь и не нужно — цифры приходят посчитанными.
- **Провайдер регистрируется всегда, даже без ключа.** Открытый вопрос 5 остаётся открытым, поэтому установка без ключа обязана подниматься и отдавать все метрики; она получает `UnconfiguredSummaryProvider`, который отказывает, а не отсутствующую регистрацию, которая падала бы при resolve.
- **Гонка Приложения N №15 закрыта повторным чтением.** Два вызывающих сгенерировали один отчёт одновременно, проигравший ловит нарушение `ux_ai_summaries_cache_key` и читает строку победителя: оба спрашивали одно и то же, и ответ на это уже есть.
- **Провал не кэшируется.** Строка со `Status='Failed'` записывается и коммитится (`AI-021` требует аудита каждого вызова), но кэшем не считается: следующий запрос пробует снова, а не переигрывает ошибку весь период.
...
**Выполнено.** 831 тест зелёные (310 unit / 64 arch / 457 integration), из них 31 — по этой фазе.

Замечания по реализации:
- **Трассировка Приложения J проверяется тестом, а не глазами.** `IsolationTraceabilityTests` сканирует исходники тестов и падает, если хоть один из `TEST-TEN-001`…`TEST-TEN-040` не покрыт. Список в документе тихо ветшает; CI-гейт — нет. На момент старта фазы не покрытыми оказались 13 случаев: `004`, `006`, `009`, `016`, `022`, `023`, `024`, `026`, `027`, `028`, `036`, `038`, `039` — все закрыты.
- **`TEST-TEN-035` исключён явно, а не пропущен.** Он проверяет отмену запросов и очистку кэша при переключении branch selector — поведение SPA, которого в этом репозитории нет. Исключение объявлено в коде теста с обоснованием; его серверная половина — `TEST-TEN-036` — покрыта.
- **Найден и закрыт пробел в реализации `TEST-TEN-022`.** Счётчика `scheduler_skipped_total{reason="branch_inactive"}` не существовало: цепочка `TEN-050` отбрасывала неактивный филиал молча. «Сегодня в Худжанде нет предложений» и «Худжанд деактивирован» давали одинаково пустой результат, и алерт `OBS-012` не мог отличить одно от другого. Добавлен проход, считающий пропуски с причиной по порядку разрыва цепочки.
- **`TEST-TEN-004` реализован как свойство неотличимости, а не как поиск.** Подстрочного поиска в контракте Release 1.0 нет; ближайшее, чем управляет вызывающий, — значение фильтра. Тело ответа сравнивается целиком (за вычетом `instance` и `traceId`, различающихся у любых двух запросов), потому что раскрытие окажется в том поле, которое добавят позже.
- **`/metrics` появился только сейчас — его не было вовсе.** Экспорт сделан пакетом `prometheus-net.AspNetCore` поверх уже существующих инструментов `System.Diagnostics.Metrics`, поэтому определения метрик остались одни, а не разошлись на два набора.
- **`http_requests_total` и `http_request_duration_seconds` пишет приложение, а не фреймворк.** Имена серий — часть контракта, на который написаны алерты 30.3; переименование при скрейпе перенесло бы контракт в конфигурацию коллектора. Label `route` — **шаблон** маршрута, никогда путь: путь содержит идентификаторы, то есть и неограниченную кардинальность, и раскрытие на эндпоинте без аутентификации.
- **`TEST-SEC-023` проверяет реальный скрейп, а не исходники.** Правило `OBS-010` — про то, что покидает процесс; label, добавленный пакетом, фреймворком или чужим helper'ом, — ровно тот случай, который проверка по коду пропустит. Labels заданы **allow-list'ом** (`MetricLabels`), а не blocklist'ом: blocklist ловит только те идентификаторы, о которых кто-то вспомнил.
- **Границей `/metrics` выбрана сеть, а не токен.** Коллектор не носит токен, поэтому эндпоинт анонимен; `MetricsOptions` по умолчанию пускает loopback и три приватных диапазона, а чужому адресу отвечает 404 — не 403, по той же логике `TEN-006`.
- **Labels несут `Organization.Slug` и `Branch.Code`, а не UUID** (`OBS-011`). Резолвер кэширует их на 30 минут: счётчики стоят на путях запроса, и обращение к БД на каждый отклонённый запрос превратило бы метрику в усилитель отказа.
- **Гейджи `TEN-096` читают снапшот, а не БД.** Observable gauge опрашивается на скрейпе, и запрос внутри callback'а завесил бы скрейп — именно тогда, когда коллектор нужен больше всего.
- **Проверки `TEN-095` исполняются тестом.** SQL-файл, который никто не запускает, ломается о переименованную колонку и обнаруживается это при восстановлении — в худший из возможных моментов. Тест читает сам файл (а не копию запросов) и проверяет оба свойства: на здоровой инсталляции пусто, на подложенном нарушении — не пусто.
- **Тест, роняющий индекс, возвращает его в `finally`.** Контейнер Postgres — коллекционная фикстура на весь прогон; оставленный без `ux_branches_single_head_office` он молча обессмыслил бы `TEST-TEN-032` в следующем же тесте.
...
| 5 | Anthropic API key и бюджет | ⏳ Открыт, **не блокирует** | Фаза 16 закрыта без ответа: провайдер `AnthropicSummaryProvider` реализован, а установка без ключа поднимается, отдаёт все метрики и отвечает 503 `AI_PROVIDER_UNAVAILABLE` только на `/reports/ai-summary`; `/health/ready` показывает AI как degraded, но не Unhealthy (`AI-019`). Ключ включается одной переменной `AI__API_KEY` плюс `AI__ENABLED=true`. Для бюджета экспортируется метрика `ai_tokens_total` с разбивкой по модели и направлению — alert настраивается, как только известна сумма |
