...
| 8 | **Composite FK на исполнителя и постановщика против переводов пользователей** | ⏳ Требует решения | Приложение M требует FK на `users(id, organization_id, branch_id, category_id)`, а разделы 15.2 и 39.6 меняют ровно эти поля при immutable scope Assignment. Одновременно выполнить оба требования нельзя. Реализовано триггером с теми же именами ограничений и тем же SQLSTATE — прямой INSERT отклоняется БД, переводы работают. Если заказчик предпочтёт сохранить FK буквально, придётся запретить перевод любого пользователя, у которого есть хотя бы одна задача **в любом статусе**, что делает таблицу последствий в 15.2 и раздел 39.7 недостижимыми |
